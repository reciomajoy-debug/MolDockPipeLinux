...
# --- Docking overlay policy (preserve Modules 1–3; overlay only docking fields) ---
DOCKING_FIELDS = {
    "vina_status","vina_score","vina_pose","vina_reason",
...
PRESERVE_FIELDS = {
    ...
    "sdf_status","sdf_path","sdf_reason",
    "pdbqt_status","pdbqt_path","pdbqt_reason",
    "tools_rdkit","tools_meeko","created_at"
}

def _backup_manifest(path: Path):
    try:
        if path.exists():
            bdir = path.parent / "backups"
            bdir.mkdir(parents=True, exist_ok=True)
            ts = datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00","Z").replace(":","").replace("-","")
...
def _overlay_docking(base_row: dict, shard_row: dict) -> bool:
    """Copy only docking-owned fields from shard -> base if shard has non-empty values.
    Returns True if anything changed; never blank out existing with empty strings."""
    changed = False
    ...
    if _ts_ord(shard_row.get("updated_at","")) >= _ts_ord(base_row.get("updated_at","")):
        base_row["updated_at"] = shard_row.get("updated_at") or now_iso()
        changed = True
    return changed

def _pick_best_shard_rows(shard_rows: List[dict]) -> Dict[str, dict]:
    """Deduplicate shard rows per id.
    Prefer rows with vina_status DONE and lowest vina_score; else latest updated_at."""
    ...
    for rid, group in by_id.items():
        ...
        if done:
            best[rid] = min(done, key=lambda g: _safe_float(g.get("vina_score","")))
        else:
            best[rid] = max(group, key=lambda g: _ts_ord(g.get("updated_at","")))
    return best
def now_iso() -> str:
...
def _merge_into(dest: Dict[str, dict], src_rows: List[dict]) -> Tuple[int,int]:
    """Overlay docking fields from shards into dest without clobbering earlier-module data.
    Returns (added, updated) counts."""
    added, updated = 0, 0
    best_by_id = _pick_best_shard_rows(src_rows)
    for rid, srow in best_by_id.items():
        if rid not in dest:
            row = {k: "" for k in MANIFEST_FIELDS}
            row["id"] = rid
            _overlay_docking(row, srow)
            if not row.get("updated_at"): row["updated_at"] = now_iso()
            if not row.get("created_at"): row["created_at"] = now_iso()
            dest[rid] = row; added += 1
        else:
            base = dest[rid]
            if _overlay_docking(base, srow):
...
def preflight_consolidate() -> None:
    ...
    per_gpu_files = [p for p in sorted(DIR_STATE.glob("manifest_gpu*.csv")) if not str(p).endswith(".tmp")]
...
def main():
    ...
    try:
        ...
        b_add, b_upd = backfill_from_results(merged)
        _backup_manifest(FILE_MANIFEST)
        save_manifest(FILE_MANIFEST, merged)
        rebuild_summaries(merged)
